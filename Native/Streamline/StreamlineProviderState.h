...
namespace rba_sl {
enum class FramePhase { Empty, Simulation, SimulationEnded, Render, RenderEnded, Submitted };
struct FrameOrder {
    uint32_t id = 0;
    FramePhase phase = FramePhase::Empty;
    bool callPending = false;
};
struct FrameLedger {
    std::array<FrameOrder, 3> frames{};
    uint32_t newest = 0;
    int Find(uint32_t id) const {
        for (size_t i = 0; i < frames.size(); ++i) if (frames[i].phase != FramePhase::Empty && frames[i].id == id) return static_cast<int>(i);
        return -1;
    }
    uint32_t Reserve(uint32_t id, int& slot) {
        slot = -1;
        if (!id || id <= newest) return RbaSlFgInvalidOrder;
        for (size_t i = 0; i < frames.size(); ++i) if (frames[i].phase == FramePhase::Empty) { slot = static_cast<int>(i); break; }
        if (slot < 0) return RbaSlFgBusy;
        newest = id; frames[slot] = {id, FramePhase::Simulation, true}; return RbaSlFgOk;
    }
    uint32_t Enter(uint32_t id, FramePhase expected, int& slot) {
        slot = Find(id);
        if (slot < 0 || frames[slot].phase != expected) return RbaSlFgInvalidOrder;
        if (frames[slot].callPending) return RbaSlFgBusy;
        frames[slot].callPending = true; return RbaSlFgOk;
    }
    void Complete(int slot, FramePhase phase) { frames[slot].phase = phase; frames[slot].callPending = false; }
    uint32_t Discard(uint32_t id) {
        const int slot = Find(id);
        if (slot < 0 || frames[slot].phase == FramePhase::Submitted) return RbaSlFgInvalidOrder;
        if (frames[slot].callPending) return RbaSlFgBusy;
        frames[slot] = {}; return RbaSlFgOk;
    }
};
...
inline bool CameraValid(const RbaSlFgCamera& camera) {
