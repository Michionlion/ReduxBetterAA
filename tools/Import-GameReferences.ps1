...
# Use the editor/package copies where available. Keep game references local;
# neither the source repository nor the release archive includes these DLLs.
...
Get-ChildItem -LiteralPath $packageCacheRoot -Filter '*.dll' -File -Recurse |
    ForEach-Object { [void]$providedAssemblies.Add($_.Name) }
Get-ChildItem -LiteralPath $packageCacheRoot -Filter '*.asmdef' -File -Recurse |
    ForEach-Object {
        $definition = Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json
        [void]$providedAssemblies.Add($definition.name + '.dll')
    }

New-Item -ItemType Directory -Path $packageRoot -Force | Out-Null
# This generated directory must not retain references from an older game/package set.
Get-ChildItem -LiteralPath $packageRoot -File | Where-Object {
    $_.Name.EndsWith('.dll') -or $_.Name.EndsWith('.dll.meta')
} | Remove-Item -Force
...
try {
    foreach ($assembly in Get-ChildItem -LiteralPath $managedRoot -Filter '*.dll' -File -Recurse) {
        ...
        $metadata = ($pluginImporterTemplate -f $guid) + "`n"
...
Write-Host 'Build references are ready. Better AA does not need the full game asset import.'
